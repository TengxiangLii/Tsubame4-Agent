...
- Commands run under a login shell (Grid Engine on TSUBAME4 resolves its
...
def get_frontend() -> Computer:
    """The (cached) Computer targeting the TSUBAME4 front-end (login) node."""
    return Computer(
        template="#!/bin/bash -l",
    ...
    with contextlib.redirect_stdout(sys.stderr):
        try:
            ...
            if not config.CONFIG_PATH.exists():
                raise RuntimeError(
                    "Plugin not configured — run the 'tsubame-configuring' skill to "
